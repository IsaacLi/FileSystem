...
short int UserInterface::GetChoice () {
	...
	cin >> currentChoice;
	while (!cin || ((currentChoice > 11) || (currentChoice < 0)) ) {
		cin.clear();
		cin.ignore(numeric_limits<streamsize>::max(), '\n');
...
bool UserInterface::ExecChoice(FileSys *fs) {
	...
	else if (this->choice == Open_Dir) {
		fs->OpenDir();
		return true;
	}
	else if (this->choice == Make_Dir) {
		fs->MkDir();
		return true;
	}
	else {
...
void UserInterface::PrintMenu () const {
	...
	cout << "(9): Open existing file Dir from disc\n";
	cout << "(10): Make a new Dir\n";
	cout << "(11): Exit\n";
}
